...
# (robust JSON parsing + recursive slide list finder)
# ============================================================
...
def _find_slide_list(obj):
    """
    Recursively search for a list of slide-like dicts.
    A "slide-like" dict just needs to be a dict (we'll clean fields later).
    """
    ...
    if isinstance(obj, dict):
        ...
        if "slides" in obj and isinstance(obj["slides"], list):
            if all(isinstance(x, dict) for x in obj["slides"]):
                return obj["slides"]
        # search all values
        for v in obj.values():
