...
# (robust JSON parsing + tolerant slide extraction)
# ============================================================
...
def _find_slide_list(obj):
    """
    Recursively search for a list of slide-like elements.

    We are deliberately tolerant:

    - If we see a list whose elements are dict / str / list → treat as slide list.
    - Else, keep recursing into nested structures.
    """
    if isinstance(obj, list):
        if all(isinstance(x, (dict, str, list)) for x in obj):
            return obj
        for x in obj:
            found = _find_slide_list(x)
...
def _coerce_to_slide_dict(item, idx: int):
    """
    Turn any kind of element into a {title, bullets, visual_required, visual_prompt} dict.
    ...
    if isinstance(item, dict):
        ...
        # If bullets is a single string, wrap in list
        if isinstance(bullets, str):
            bullets = [bullets]
        elif not isinstance(bullets, list):
            bullets = []

        # If title missing, try to guess from other keys
        if not title:
            # pick any key that looks like a title-ish thing
            for k in ["heading", "topic", "name"]:
                if k in item:
                    title = str(item[k])
                    break
        if not title:
    ...
    if isinstance(item, str):
        # whole string becomes bullets; title is first 40 chars
        txt = item.strip()
        return {
...
# ------------------------------------------------------------
# LLM PLAN GENERATOR (JSON-enforced, robust + tolerant)
# ------------------------------------------------------------
def call_llm_plan(
    prompt,
    style,
    ...
    try:
        ...
        slides_raw = _find_slide_list(parsed) if parsed is not None else None

        if not isinstance(slides_raw, list) or not slides_raw:
            raise ValueError("Could not find slide list in JSON")

        # Coerce every element into a slide dict
        cleaned = []
        for idx, item in enumerate(slides_raw):
            cleaned.append(_coerce_to_slide_dict(item, idx))
        ...
        # Only a WARNING, but we still return a valid plan via fallback.
