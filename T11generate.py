...
# (robust JSON handling for all text densities)
# ============================================================
...
def call_llm_plan(
    ...
    # We try to force JSON via response_format, but also handle plain JSON text.
    sys_prompt = (
    ...
    try:
        ...
        # Some deployments support .parsed, some don't. Handle both.
        parsed = getattr(resp.choices[0].message, "parsed", None)
        if parsed is None:
            raw = resp.choices[0].message.content
            parsed = safe_json_load(raw)

        # --- Accept both dict + list shapes ---
        if isinstance(parsed, dict):
            ...
            raise ValueError(f"Parsed plan has unexpected type: {type(parsed)}")
