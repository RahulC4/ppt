...
# ------------------------------------------------------------
def call_llm_plan(prompt, references_text=None, num_slides: int = 5):
    """
...
# ------------------------------------------------------------
def parse_text_plan(text: str, num_slides: int):
    """
    ...
    for ph in slide.placeholders:
        if ph.placeholder_format.type in (PH.TITLE, PH.CENTER_TITLE, PH.SUBTITLE):
            if getattr(ph, "has_text_frame", False):
                ph.text = text
                return
    # If nothing is found, we silently skip (no crash).


def _clear_non_title_text(slide):
    """Clear all text on a slide except the title (used on title slide to remove speaker name etc.)."""
    title_shape = slide.shapes.title if hasattr(slide.shapes, "title") else None
    for shp in slide.shapes:
        if getattr(shp, "has_text_frame", False) and shp is not title_shape:
            shp.text = ""
...
def _set_agenda_bullets(slide, bullets):
    """
    Special bullet setter for the Agenda slide:
    - Keep design & image.
    - Clear ALL non-title text boxes.
    - Use the first text box for the agenda bullets.
    """
    title_shape = slide.shapes.title if hasattr(slide.shapes, "title") else None
...
def build_corporate_ppt(plan, image_required: bool):
    ...
    Structure:
      1. Title slide (existing template slide 1, text + date overwritten, speaker cleared)
      2. Agenda slide (existing template slide 2; bullets overwritten with our sections,
         image & design preserved)
      3..N+2. Content slides (from plan, using standard Title+Content layout, with optional images)
      Last. Thank-you slide (from template's thank-you layout; no image)
    """
    ...
    if len(prs.slides) < 2:
        logger.warning("Corporate template has fewer than 2 slides; fallback to default.")
        return build_default_ppt(plan, image_required)

    # Use a standard Title + Content layout from this theme for content slides
    # (this is usually index 1).
    content_layout = prs.slide_layouts[1]

    # Capture thank-you layout from last slide before we delete it
    thanks_layout = prs.slides[-1].slide_layout

    # Remove all slides except index 0 (title) and 1 (agenda)
    ...
    _clear_non_title_text(title_slide)  # remove speaker name / extra text

    # ---- Agenda slide (slide 1, existing design) ----
    agenda_slide = prs.slides[1]
    # Keep the template's "Agenda" title/text style, only replace bullets
    agenda_items = [s["title"] for s in plan if s.get("title")]
    if not agenda_items:
        agenda_items = [f"Section {i + 1}" for i in range(len(plan))]
    _set_agenda_bullets(agenda_slide, agenda_items)

    # ---- Content slides (newly generated) ----
    for sp in plan:
    ...
    # ---- Thank-you slide at the end (from template layout, no image) ----
