...
def build_corporate_ppt(plan, image_required: bool):
    ...
    Structure:
      1. Title slide (existing template slide 1, text + date overwritten)
      2. Agenda slide (existing template slide 2; bullets overwritten with our sections,
         image & design preserved)
      3..N+2. Content slides (from plan, using a content layout, with optional images)
    ...
    # Need at least title, agenda, and some content layout
    if len(prs.slides) < 2:
        logger.warning("Corporate template has fewer than 2 slides; fallback to default.")
        return build_default_ppt(plan, image_required)

    # Use the third slide's layout (if present) as a generic content layout,
    # otherwise fall back to a standard title+content layout.
    if len(prs.slides) >= 3:
    ...
    # We keep their full design (images, shapes, etc.).
    ...
    # Keep whatever the template has as title (e.g., "Agenda"), but we could force it:
    ...
    _set_bullets(agenda_slide, agenda_items)  # only bullets are replaced
