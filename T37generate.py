...
def _set_bullets(slide, bullets):
    """Fill BODY text placeholder (or closest equivalent) with bullet points."""
    body_shape = None
    ...
    # Fallback: any non-title shape with text_frame
    if body_shape is None:
        for shp in slide.shapes:
            if shp is slide.shapes.title:
                continue
    ...
    if body_shape is None:
        logger.warning("No suitable body placeholder found; skipping bullets")
        return
...
def _add_image_to_slide(slide, img_path: str):
    """Place image on the right side, shrinking body area to leave space."""
    try:
        body_shape = None
        for ph in slide.placeholders:
...
# ------------------------------------------------------------
# CORPORATE TEMPLATE BUILDER (FIXED)
# ------------------------------------------------------------
def build_corporate_ppt(plan, image_required: bool):
    ...
    Structure:
      1. Use template's existing first slide as title (we overwrite text + date)
      2. Use template's existing second slide as Agenda (title + bullets)
      3..N+2. New content slides based on a generic content layout
      Last.  New Thank-you slide created from the template's last layout

    Only content slides (3..N+2) can get generated images.
    """
    ...
    # Identify layouts / slides
    title_slide = prs.slides[0]
    agenda_slide = prs.slides[1] if len(prs.slides) > 1 else None
    thanks_layout = prs.slides[-1].slide_layout

    # Choose a content layout: must have TITLE + BODY
    content_layout = None
    for layout in prs.slide_layouts:
        has_title = any(
            ph.placeholder_format.type in (PH.TITLE, PH.CENTER_TITLE)
            for ph in layout.placeholders
        )
        has_body = any(
            ph.placeholder_format.type == PH.BODY for ph in layout.placeholders
        )
        if has_title and has_body:
            content_layout = layout
            break
    if content_layout is None:
        # Fallback: second layout (usually title+content)
        content_layout = prs.slide_layouts[1]

    # Remove ALL slides except the first two (title+agenda)
    if len(prs.slides) > 2:
        for idx in reversed(range(2, len(prs.slides))):
            _delete_slide(prs, idx)

    # Re-fetch (collection was mutated)
    title_slide = prs.slides[0]
    agenda_slide = prs.slides[1] if len(prs.slides) > 1 else None

    # ---- Title slide: overwrite title + date ----
    ...
    # Clear speaker placeholders if any (optional)
    for ph in title_slide.placeholders:
        if ph is title_slide.shapes.title:
            continue
        if getattr(ph, "has_text_frame", False):
            ph.text = ""

    # ---- Agenda slide: title + bullets ----
    ...
    # ---- Content slides (based on content_layout), with optional images ----
    for sp in plan:
    ...
    # ---- Thank-you slide at the end (no image, but update dates) ----
    thanks_slide = prs.slides.add_slide(thanks_layout)
    _update_date_placeholders(thanks_slide)
