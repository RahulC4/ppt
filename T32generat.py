...
# + Visual template from Azure Blob
# ============================================================
...
from azure_blob_utils import (
    upload_ppt_to_blob,
    upload_json_to_blob,
    download_source_ppt_to_temp,  # NEW
)
...
# ------------------------------------------------------------
# PPT BUILDER (supports optional template path)
# ------------------------------------------------------------
def _clear_all_slides(prs: Presentation):
    """
    Remove all existing slides from a Presentation while keeping the theme.
    Uses internal python-pptx structures – OK for our use case.
    """
...
def build_ppt(slides, template_path: str | None = None):
    """
    Build the PPT:
    - If template_path is provided → start from that PPT's theme and
      clear its slides.
    - Else → start from a blank default Presentation.
    """
    ...
    if template_path:
        try:
            ...
            logger.info(f"Building PPT using template: {template_path}")
        except Exception:
            logger.exception("Failed to open template PPT, using blank presentation")
            prs = Presentation()
    else:
        prs = Presentation()

    for sp in slides:
        # Standard "Title and Content" layout – index 1 in most templates
        ...
        # Body placeholder
        # If the chosen layout doesn't have placeholder[1] we skip bullets gracefully
        body = None
        try:
        ...
        if not img_path:
            # NO IMAGE → if we have a body placeholder, make it full width
            if body is not None:
                body.left = Inches(0.5)
                body.width = prs.slide_width - Inches(1.0)
            continue

        # IMAGE PRESENT → split layout (text left, image right)
        if body is not None:
            body.left = Inches(0.5)
        ...
        try:
            ...
            top = (body.top if body is not None
                   else slide.shapes.title.top + slide.shapes.title.height + Inches(0.3))
...
def generate_presentation(
    ...
    text_density=None,
    tag_filters=None,
    template_style=None,  # NOW: blob name of selected template PPT (or None)
):
    """
    ...
    plan = call_llm_plan(
        ...
        theme=theme,
        text_density=text_density,   # usually None → LLM decides
    )
    ...
    # 5b) Resolve template PPT from blob, if any
    template_blob_name = (
        None
        if not template_style
        or template_style in ("Plain (No template)", "Plain", "Auto")
        else template_style
    )

    template_path = None
    if template_blob_name:
        try:
            template_path = download_source_ppt_to_temp(template_blob_name)
        except Exception:
            # already logged in azure_blob_utils
            template_path = None

    # 6) Build PPT
    out_path = build_ppt(slides, template_path=template_path)
