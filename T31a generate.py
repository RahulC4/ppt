...
def _get_body_shape(slide):
    """
    Try to find a good 'body/content' shape:
    1) Any placeholder with a text frame that isn't the title.
    2) Otherwise any shape with a text frame that isn't the title.
    """
...
def build_ppt(slides):
    ...
    for sp in slides:
        ...
        # Find or create a proper body shape
        body = _get_body_shape(slide)
        if body is None:
            # Create a new textbox spanning most of the slide
            left = Inches(0.5)
            ...
            # Position the body below the title and full width by default
        ...
        # If we have an image, shrink body and place image on the right
        if sp.get("image_path"):
            try:
                # Shrink body to left side
                body.width = prs.slide_width - Inches(4)
                img_left = body.left + body.width + Inches(0.3)
                img_top = body.top
