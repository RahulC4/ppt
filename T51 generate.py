# ============================================================
# generate_ppt.py – NO JSON | Images Checkbox | Corporate Template (FIXED)
# ============================================================
...
# ✅✅✅ DATE FIX (NOW FORCES CURRENT MONTH + YEAR)
def update_date(slide):
    current = datetime.now().strftime("%B %Y")
    for shp in slide.shapes:
        if shp.has_text_frame:
            if "date" in shp.text.lower() or re.search(r"\b20\d{2}\b", shp.text):
                shp.text = current


# ✅✅✅ BULLET FIX (FORCES REAL BULLETS)
...
# ------------------------------------------------------------
# ✅✅✅ FIXED CORPORATE BUILDER (NON IMAGE)
# ------------------------------------------------------------
def build_corporate_ppt(plan):
    ...
    update_date(title_slide)  # ✅ DATE FIX APPLIED

    for shp in title_slide.shapes:
        if shp.has_text_frame and shp.text != plan[0]["title"] and not re.search(r"\b20\d{2}\b", shp.text):
            shp.text = ""
    ...
    # ---- 3..N+2 CONTENT SLIDES (✅ BULLETS FIXED)
    for sp in plan:
