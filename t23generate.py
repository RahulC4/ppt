...
#            (FIXED + IMAGE RULES + PLACEHOLDER BULLETS)
# ============================================================
...
from pptx.enum.shapes import PP_PLACEHOLDER  # ✅ NEW
...
def call_llm_plan(prompt, references_text=None, num_slides=5):
    ...
    sys_prompt = (
        ...
        "- 3–5 sentence description line 1\n"
        "- 3–5 sentence description line 2\n"
        "- 3–5 sentence description line 3\n\n"
...
def parse_text_plan(text, num_slides):
    ...
    for block in blocks:
        ...
        # Description lines – we still expect them to start with "-"
        bullets = [l[1:].strip() for l in lines[1:] if l.startswith("-")]
        if not bullets:
            # fallback: treat remaining lines as description
            bullets = lines[1:]
...
def set_title(slide, text):
    """Set the first text-frame shape as title."""
...
def add_bullet_textbox(slide, bullets, left, top, width, height, font_size=20, color=RGBColor(0, 0, 0)):
    """Fallback textbox (used only if BODY placeholder unavailable)."""
...
def get_body_placeholder(slide):
    """
    Prefer the BODY placeholder from the template.
    If not found, fall back to any non-title text frame.
    """
    # BODY placeholder if exists
    for shp in slide.placeholders:
        if (
            getattr(shp, "is_placeholder", False)
            and getattr(shp, "has_text_frame", False)
            and shp.placeholder_format.type == PP_PLACEHOLDER.BODY
        ):
            return shp

    # Fallback to any non-title text frame
    title_shape = getattr(slide, "shapes", None)
    title_obj = getattr(slide.shapes, "title", None) if slide.shapes else None

    for shp in slide.shapes:
        if getattr(shp, "has_text_frame", False) and shp is not title_obj:
            return shp
...
# ------------------------------------------------------------
# CORPORATE BUILDER WITH PLACEHOLDER BULLETS
# ------------------------------------------------------------
def build_corporate_ppt(plan, image_required=False):
    ...
    # Remove sample slides; keep layouts
    ...
    # Very light corporate background
    ...
    body = get_body_placeholder(agenda_slide)
    if body is None:
        # fallback textbox
    ...
    # Width differs if image used
    if image_required:
        body.width = prs.slide_width - Inches(4.5)
    else:
        body.width = prs.slide_width - Inches(2)
    ...
    # ---- 3..N+2 CONTENT SLIDES (IMAGE ALLOWED) ----
    for sp in plan:
        ...
        # Width depends on image
        if image_required:
            body.width = prs.slide_width - Inches(4.5)
        else:
            body.width = prs.slide_width - Inches(2)

        # Add one bullet per description line
        for b in sp["bullets"]:
            p = tf.add_paragraph()
            p.text = b
            p.level = 0
