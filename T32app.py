...
from azure_blob_utils import upload_source_ppt_to_blob, list_source_ppts
...
st.write(
    ...
    "Upload sample PPTs, pick a visual template, and let the model enhance – not invent from scratch."
)
...
# ============================================================
# 1️⃣ UPLOAD SAMPLE PPTs (Knowledge Base + Templates)
# ============================================================
st.subheader("1️⃣ Upload Sample PPT Files (Knowledge Base)")

uploaded_files = st.file_uploader(
    "Upload .pptx files to use as references/templates:",
    type=["pptx"],
    ...
    with st.spinner("Uploading & indexing PPTs..."):
        for upl in uploaded_files:
            try:
                ...
                # Upload to blob (source/sample container)
                upload_source_ppt_to_blob(bytes_data, blob_name)
                # Ingest into Chroma (text + embeddings)
...
with col2:
    # Templates from Azure Blob (source/container)
    blob_templates = list_source_ppts()  # returns list of blob names
    template_options = ["Plain (Default)"] + blob_templates

    template_style = st.selectbox(
        "Visual Template (from your sample PPTs)",
        template_options,
        index=0,
    )

with col3:
    image_required = st.checkbox("🖼 Generate images for all slides", value=False)
        ...
        with st.spinner("Generating final PowerPoint..."):
            try:
                ppt_path, log = generate_presentation(
                    ...
                    template_style=template_style,   # blob name or "Plain (Default)"
