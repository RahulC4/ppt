...
# ✅ WITH UI-CONTROLLED IMAGE GENERATION
# ============================================================
...
def call_llm_plan(prompt, style, design_context=None, references_text=None, num_slides=None, theme=None, text_density=None):
    ...
    try:
        ...
        logger.warning(f"Fallback plan used: {e}")
        n = num_slides or 3
        return [{
            "title": f"Slide {i + 1}",
...
def generate_presentation(
    ...
    tag_filters=None,
    template_style=None,
    image_required=True,   # ✅ GLOBAL IMAGE TOGGLE
):
    ...
    for idx, sp in enumerate(plan):
        img_path = None
        if image_required:
            img_prompt = sp.get("visual_prompt") or f"Professional illustration for {sp.get('title', f'Slide {idx+1}')}"
            img_path = generate_visual_image(img_prompt)
