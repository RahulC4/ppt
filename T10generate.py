...
# (stable JSON via response_format, density handled in Python)
# ============================================================
...
# ------------------------------------------------------------
# LLM PLAN GENERATOR (JSON-enforced)
# ------------------------------------------------------------
def call_llm_plan(
    prompt,
    style,
    ...
    # We force JSON using response_format={"type": "json_object"}.
    sys_prompt = (
        "You are a presentation planner.\n"
        "You MUST respond with a single JSON object ONLY, with this shape:\n\n"
        "{\n"
        '  "slides": [\n'
        '    {\n'
        '      "title": "string",\n'
        ...
        "}\n\n"
        "Rules:\n"
        "- No markdown, no comments, no text outside that JSON object.\n"
    ...
    try:
        ...
        # New SDK: parsed JSON is available here when response_format is json_object
        ...
        if not isinstance(slides_raw, list) or not slides_raw:
            raise ValueError("slides[] missing or empty")
