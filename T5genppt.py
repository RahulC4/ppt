# ============================================================
# generate_ppt.py  (FINAL – Corporate Layout: Left Text / Right Image)
# ============================================================
...
def parse_user_intent(prompt: str):
    ...
    theme_words = ["corporate", "modern", "minimal", "professional", "dark", "light"]
...
def call_llm_plan(prompt, style, design_context, references_text,
    ...
    sys_prompt = (
        ...
        "Your job is to create a structured slide plan in STRICT JSON ONLY.\n"
        "Format:\n"
        "[ {\"title\": str, \"bullets\": [str], \"visual_required\": bool, \"visual_prompt\": str } ]\n"
        "If the user asks for images, ALWAYS set visual_required=true.\n"
        "Do NOT include text/labels inside images.\n"
        "Stay concise and business-like.\n"
    )
    ...
    try:
        ...
        if not plan:
            logger.warning("Invalid slide JSON, fallback used.")
...
def generate_visual_image(prompt: str):
    ...
    img_prompt = (
        "Minimal, clean, professional illustration relevant to the topic. "
...
def build_ppt(slides):
    ...
    for s in slides:
        ...
        # ---------------------------------------------------------
        # CORPORATE LAYOUT FIX: SHRINK CONTENT AREA FOR IMAGE
        # ---------------------------------------------------------
        try:
            body = slide.placeholders[1]

            new_width = prs.slide_width - Inches(4.0)  # Leave right space for image
...
def generate_presentation(prompt: str, style="Auto", requested_num_slides=None,
    ...
    logger.info(f"Loaded {len(design_context)} design files and {len(reference_text)} text snippets.")
