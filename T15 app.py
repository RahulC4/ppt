...
from generate_ppt_auto import generate_presentation_auto, call_llm_plan_auto
...
# ✅ SAFE Session State Init (DO NOT RESET ON RERUN)
...
st.write(
    ...
    "Upload sample PPTs, and let the model enhance and create PPT."
)
...
with st.sidebar:
    ...
    if kb_files:
        for blob_name in kb_files:
            ...
            with col_b:
                if st.button("🗑️", key=f"delete_{blob_name}", help="Delete this template"):
                    try:
...
prompt = st.text_area(
    ...
    placeholder="Example: Create a presentation deck about Proposal approach for carefirst global design.",
    height=150,
)
        ...
        with st.spinner("Searching knowledge base & generating preview..."):
            try:
                ...
                if not refs:
                    ...
                    if not plan:
                        st.warning("⚠️ Not enough relevant content.")
                    else:
        ...
        with st.spinner("Generating final PowerPoint , usually takes less than 60s..."):
            try:
                ...
                if log.get("error"):
                    ...
                    safe_prompt = prompt.strip().split(" ")[:4]
                    safe_prompt = "_".join(safe_prompt)

                    timestamp = datetime.now().strftime("%d_%b_%H-%M")
                    display_name = f"{safe_prompt}_{timestamp}.pptx"
...
# ============================================================
# 📂 THIS SESSION'S GENERATED PPTs (SORTED ✅)
# ============================================================
st.subheader("📂 Generated PPTs")
