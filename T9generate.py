...
# (JSON-stable across Minimal / Concise / Detailed / Extensive)
# ============================================================
...
def _extract_json_array(raw: str):
    """
    Try to extract a JSON array from the model response, even if it
    wraps it with extra prose. Returns list or None.
    """
    if not raw:
        return None

    # First try safe_json_load directly
    try:
    ...
    # Then try to slice between first '[' and last ']'
    try:
        start = raw.find("[")
        end = raw.rfind("]")
...
# ------------------------------------------------------------
# LLM PLAN GENERATOR (JSON only, density handled later)
# ------------------------------------------------------------
def call_llm_plan(
    prompt,
    style,
    ...
    # Keep instructions simple so JSON stays valid
    sys_prompt = (
        "You are a presentation planner.\n"
        "Return STRICT JSON ONLY in this exact format:\n"
        "[{\"title\": str, \"bullets\": [str], "
        "\"visual_required\": bool, \"visual_prompt\": str }]\n"
        "Do NOT include markdown, comments, or any text outside the JSON array.\n"
        "Each bullet must be a plain string (no extra quotes, bullet symbols, or numbering).\n"
        "If the user asks for images, set visual_required=true.\n"
        "Do NOT put any text or labels inside the images themselves.\n\n"
    ...
    try:
        ...
        plan_raw = _extract_json_array(raw)
